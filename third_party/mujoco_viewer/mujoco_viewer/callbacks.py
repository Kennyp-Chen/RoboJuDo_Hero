        ...
        if self._left_double_click_pressed or self._right_double_click_pressed:
            ...
            # set body selection
                ...
                if selbody >= 0:
                    ...
                    # 20260323
                    #新版本MuJoCo对skinselect的数据类型要求更严格
                    # mjv_select函数返回的selskin数据格式发生了变化
                    # 修复双击机器人就崩溃问题
                    self.pert.skinselect = int(selskin[0, 0]) if hasattr(selskin, 'shape') and selskin.shape == (1, 1) else int(selskin)
                    # compute localpos
