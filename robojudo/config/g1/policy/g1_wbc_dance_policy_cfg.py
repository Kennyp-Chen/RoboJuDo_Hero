...
class G1WbcDanceDoF(DoFConfig):
    """G1 joint config for WBC Dance policy (policy order).
...
class G1WbcDancePolicyCfg(WbcDancePolicyCfg):
    """G1-specific configuration for WBC Dance policy."""
    robot: str = "g1"
    ...
    history_obs_dims: dict[str, int] = {
        "ang_vel": 3,
        "gravity": 3,
        "dof_pos": G1WbcDanceDoF().num_dofs,
        "dof_vel": G1WbcDanceDoF().num_dofs,
        "actions": G1WbcDanceDoF().num_dofs,
    }
