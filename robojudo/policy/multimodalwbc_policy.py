...
class MultiModalWBCPolicy(Policy):
    cfg_policy: MultiModalWBCPolicyCfg

    def __init__(self, cfg_policy: MultiModalWBCPolicyCfg, device):
        ...
        if cfg_policy_new.use_modelmeta_config:
            logger.info("[MultiModalPolicy] Using modelmeta as config ...")
    ...
    def reset(self):
        ...
        if self.use_motion_from_model:
            self.pbar = ProgressBar(f"MultiModalWBC {self.cfg_policy.policy_name}", self.max_timestep)
        else:
            self.pbar = None
    ...
    def _get_command(self, env_data, ctrl_data):
        if not self.use_motion_from_model:
            # assert "MultiModalWBCCtrl" in ctrl_data, "MultiModalWBCCtrl not found in ctrl_data"
            # command = ctrl_data.get("MultiModalWBCCtrl")
