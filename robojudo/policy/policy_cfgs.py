...
class UnitreeMjlabVelocityPolicyCfg(PolicyCfg):
    ...
    # max_cmd: list[float] = [1.0, 1., 1.3]  # [lin_vel_x, lin_vel_y, ang_vel_z] from deploy.yaml
    # # max_cmd: list[float] = [0.8, 0.5, 1.58]

    # # Command mapping from deploy.yaml（修复为3个值）
    # # commands_map: list[list[float]] = [
    # #     [-0.5, 0.0, 0.8],  # forward: [-0.5, 0.0, 1.0] from deploy.yaml
    # #     [0.6, 0.0, -0.5],  # lateral: [-0.5, 0.0, 0.5] from deploy.yaml  
    # #     [1.2, 0.0, -1.2],  # angular: [-1.0, 0.0, 1.0] from deploy.yaml
    # # ]
    
    # commands_map: list[list[float]] = [
    #     [-0.5, 0.0, 0.6],  # forward: [-0.5, 0.0, 1.0] from deploy.yaml
    #     [0.6, 0.0, -0.5],  # lateral: [-0.5, 0.0, 0.5] from deploy.yaml  
    #     [1., 0.0, -1.],  # angular: [-1.0, 0.0, 1.0] from deploy.yaml
    # ]
    # #左走大不稳定
    # #左转不稳定再小店？
    # # commands_map: list[list[float]] = [
